...
void ClientModel::updateAlert(const QString &hash, int status)
{
    ...
    if(status == CT_NEW)
    {
        ...
        if(!alert.IsNull())
        {
            emit error(tr("Network Alert"), QString::fromStdString(alert.strStatusPrv), false);
        }
    }

    // Emit a numBlocksChanged when the status message changes,
    // so that the view recomputes and updates the status prv.
...
QString ClientModel::getStatusPrvWarnings() const
{
    return QString::fromStdString(GetWarnings("statusprv"));
}
